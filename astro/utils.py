...
from astro import LOAD_PLUG
from astro import CMD_LIST
import re
import logging
import inspect

handler = Config.HNDLR if Config.HNDLR else r"\."
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        # support for uniborg
        sys.modules["uniborg.util"] = astro.utils
        mod.Config = Config
        mod.borg = bot
        mod.astro = bot
        # auto-load
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"astro.plugins.{shortname}"
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["astro.plugins.assistant" + shortname] = mod
